...
mod test {
    use crate::{
        memory::{main::timer::TimerEntry, with_stable_memory_mut, MainPartition},
        NanoTimeStamp,
    };
    ...
    fn test_init_main_partition() {
        let main_partition = with_stable_memory_mut(|pm| MainPartition::init(pm));
    ...
    fn test_core_backup_partition() {
        let mut main_partition = with_stable_memory_mut(|pm| MainPartition::init(pm));
