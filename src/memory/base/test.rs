...
mod test {
    use crate::{
        memory::{base::timer::TimerEntry, with_stable_memory_mut, BasePartition},
        NanoTimeStamp,
    };
    ...
    fn test_init_main_partition() {
        let main_partition = with_stable_memory_mut(|pm| BasePartition::init(pm));
    ...
    fn test_core_backup_partition() {
        let mut main_partition = with_stable_memory_mut(|pm| BasePartition::init(pm));
